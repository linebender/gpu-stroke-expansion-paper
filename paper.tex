...
\begin{table}
    ...
    \hline
        waves.svg  & 6 $\mu$s & 6.13 $\mu$s \\
        mmark-70k  & 2.32 ms  & 6.59 ms     \\
        mmark-120k & 4.17 ms  & 11.26 ms    \\
        long path  & 4.14 ms  & 6.28 ms     \\
    \hline
    \end{tabular}
...
\begin{itemize}
    ...
    \item The pre-allocation requirement for bump-allocated line soup buffer is a limitation due to today's graphics APIs. An interesting future exploration is to look at newer GPU API features, such as mesh shaders and work graphs, for GPU-driven scheduling of the compute workload under a bounded memory footprint.
