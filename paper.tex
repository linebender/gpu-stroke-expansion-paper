...
\begin{figure*}
    ...
    \caption{Renderings of some of our test scenes created using our algorithm. All images were rendered on an Apple M1 Max at 2088x1600 resolution using our GPU pipeline. \textbf{(a)} \emph{waves.svg} from the Nehab2020 timings data set (2.94~ms); \textbf{(b)} The \emph{mmark-120k} scene with 120,000 input segments (13.89~ms); \textbf{(c)} The \emph{long path} scene with ~500k input segments and round caps (25~ms including CPU-side dashing - the GPU time to render the scene is 9~ms); \textbf{(d)}; A simple test scene showcasing various cap and join styles (1.05~ms); \textbf{(e)} The famous \emph{Ghostscript Tiger} containing both stroked and filled paths (1.23~ms).}
