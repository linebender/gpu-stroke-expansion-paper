...
\begin{enumerate}
  ...
  \item The CPU must do no work beyond the basic preperation of the input path data for GPU consumption.
\end{enumerate}
...
Our compute shader is arranged as a 1-dimensional grid, parallelized over input path segments. We chose a workgroup size of 256 as it works well over a wide range of GPUs. Each thread in the global grid is assigned to process a single path segment. We encode the individual path segments contiguously in a GPU storage buffer such that each element has 1:1 correspondance to a global thread ID in the dispatch. This layout easily scales to tens of thousands of input path segments, which can be distributed across any number of paths and subpaths.

% TODO: caps/joins figure highlighting mapping of segments, joins, and caps to threads, encoding.
...
\begin{algorithm}
  ...
  \eIf{$style$ is $fill$}{
      flatten source curve
  }{\If{you're sad}{
      just keep going
  }}
\caption{The control flow of our compute shader}
\end{algorithm}

Each thread outputs a polyline that fits a subset of the rendered subpath's outer contour, represented by the path segment. For a filled primitive, a thread only outputs the flattened approximation of the path segment. For a stroke, a thread outputs the polylines approximating multiple curves: a) the two parallel curves on both sides of the source segment, offset by the desired stroke \emph{halfwidth}, and b) the inner and outer joins that connect the path segment to the next adjacent segment. We also store metadata in our input encoding that marks the position of a path segment within the containing subpath. We use this information to determine whether to output an end cap instead of a join. We encode one additional segment designated as the \emph{stroke cap marker} for every subpath. The thread assigned to the marker segment only outputs a start cap, which completes the stroke outline.

The joins between adjacent segments need to form a continuous outline when combined with the segments. This requires that a thread have access to the tangent vector at the end of its assigned segment \emph{and} the start tangent of the next adjacent segment. We require that the input segments within a subpath are stored in order which simplifies the access to the adjacent segment down to a simple read at the next array offset. Co-operation across threads is not necessary.
...
% TODO: Talk about the inner join here. We detect inner vs outer outline based on join tangent cross product. We just draw a straight line for the inner join.
...
% TODO: not sure how much to emphasize the path tag encoding. The input processing monoids benefit the vello scene API (fragments) the TagMonoid could generally be computed on the CPU (with minimal offset tracking) It's not that relevant to the stroke processing. The main bit to mention is that it's a struct-of-arrays.
% TODO: talk about sanitization of input: closepaths are handled ahead of time, stroke cap marker.
% TODO: discuss style encoding in greater detail
...
% TODO: 3 alternative implementations: cubic->ES (for load balancing eval), cubic->ES->LineSoup (current), cubic->ES->ArcSoup
% TODO: GPUs tested, data sets. Test strategy
% TODO: divergence (reference prim count)
...
% TODO: mention dashing as future work?
