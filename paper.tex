...
\begin{figure}
    \includegraphics[scale=0.65]{nehab_gpu_timings}
    \caption{Mean GPU execution times of the compute shader that generates stroked and filled outlines, for the Nehab2020 \emph{timings} data set, in microseconds. The hatched bars show the timings for the version that outputs line primitives while the solid bars show that for arcs. Mali-G78 (the lowest end GPU we tested) converts \emph{waves} (the largest scene in the set) to lines in ~3ms. The compute shader executes at least an order of magnitude faster on the other GPUs.}
    \Description{A stacked bar chart of the GPU timings for stroke expansion from Nehab2020 timings SVGs}
    \label{fig:nehab-gpu-timings}
\end{figure}

\begin{figure}
    \includegraphics[scale=0.66]{test_scenes_gpu_timings}
    \caption{Mean GPU execution times of the compute shader for the stress test scenes.}
