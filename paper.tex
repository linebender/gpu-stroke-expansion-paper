...
Regarding dashing, previous work, particularly \citet{Kilgard2020}, has explained how to break the input path into dash segments of the specified length on GPU using a prefix sum of segment arc lengths. There are implementation tradeoffs to GPU-based dash processing, including potentially a larger number of dispatch stages. Whether moving dashing to a GPU stage is a performance win depends on the scene and other factors in the overall rendering pipeline. While we would expect further performance improvement from GPU-based dashing, we were able to achieve acceptable performance with dash processing on CPU.
